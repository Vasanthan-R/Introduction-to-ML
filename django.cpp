...
int main() 
{
    ...
    ifstream inFileRawReviewRatingsStopWords;             //stopwords
    ...
    inFileRawReviewRatingsStopWords.open("stopwords.txt");
    if (!inFileRawReviewRatingsStopWords) {
    ...
    while(inFileRawReviewRatingsStopWords>>s)
	    {
	    	stop_word+=s;
	    	stop_word+=32;
		}
	...
	inFileRawReviewRatingsStopWords.close();
	...
	float total_count[count]={0};          //count of words
	float rating[count]={0};               //individual rating of words
	...
	{
		m.insert({words[k],make_pair(rating[k],total_count[k])});	  //insert in map	
    }
